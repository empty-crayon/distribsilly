package config_test

import (
	"io/ioutil"
	"os"
	"reflect"
	"testing"

	"github.com/empty-crayon/distribsilly/config"
)

func createConfig(t *testing.T, contents string) config.Config {
	t.Helper()

	f, err := ioutil.TempFile(os.TempDir(), "config.toml")

	if err != nil {
		t.Fatalf("Couldnt create a temp file: %v", err)
	}

	defer f.Close()
	name := f.Name()
	defer os.Remove(name)

	_, err = f.WriteString(contents)

	if err != nil {
		t.Fatalf("Couldnt write config contents: %v", err)
	}

	c, err := config.ParseFile(name)

	if err != nil {
		t.Fatalf("Couldnt create parse config: %v", err)
	}

	return c
}

func TestConfigParse(t *testing.T) {
	got := createConfig(t, `[[shards]]
	name = "delhi"
	idx = 0
	address = "localhost:8080"`)

	want := config.Config{
		Shards: []config.Shard{
			{
				Name:    "delhi",
				Idx:     0,
				Address: "localhost:8080",
			},
		},
	}

	if !reflect.DeepEqual(got, want) {
		t.Errorf("The config does not match! got: %#v, want: %#v", got, want)
	}
}

func TestParseShards(t *testing.T) {
	c := createConfig(t, `
	[[shards]]
		name = "delhi"
		idx = 0
		address = "localhost:8080"
	[[shards]]
		name = "kolkata"
		idx = 1
		address = "localhost:8081"`)

	got, err := config.ParseShards(c.Shards, "delhi")

	if err != nil {
		t.Fatalf("Could not parse shards, %#v: %v", c.Shards, err)
	}

	want := &config.Shards{
		Count:  2,
		CurIdx: 0,
		Addrs: map[int]string{
			0: "localhost:8080",
			1: "localhost:8081",
		},
	}

	if !reflect.DeepEqual(want, got) {
		t.Errorf("The config does not match! got: %#v, want: %#v", want, got)
	}

}
